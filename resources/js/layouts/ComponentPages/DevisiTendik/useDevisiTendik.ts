...
export const useDevisiTendik = () => {
    ...
    const fetchData = async (currentPage = 1) => {
        try {
            setIsLoading(true);
            const res: any = await get(`/staff-division?page=${currentPage}&limit=10`);
    ...
    const handleSubmit = async (data: Omit<DevisiTendikType, 'id'>, id?: number, onSuccess?: () => void) => {
        try {
            setIsLoading(true);
            if (id) {
                const res: any = await put(`/staff-division/${id}`, data);
                setData((prev) => prev.map((p) => (p.id === id ? res.data : p)));
                await fetchData();
                onSuccess?.();
                setToast({ message: 'Devisi Tendik updated successfully', type: 'success' });
                return res;
            } else {
                const res: any = await post('/staff-division', data);
                setData((prev) => [...prev, res.data]);
                await fetchData();
                onSuccess?.();
                setToast({ message: 'Devisi Tendik created successfully', type: 'success' });
                return res;
            }
        } catch (error: any) {
            if (error?.response?.status === 500) {
                setToast({ message: 'Failed to submit Devisi Tendik', type: 'error' });
            }
