...
import { AcademicPosition, columns } from './Column';
import ModalForm from './Modal';

const breadcrumbs: BreadcrumbItem[] = [{ title: 'Academic Position', href: '/academic-position' }];
...
const AcademicPositionPage = () => {
    ...
    const fetchData = async () => {
        try {
            const res: any = await get('/academic-position');
            setData(res);
        } catch (err) {
    ...
    const handleSubmit = async (academicPosition: Omit<AcademicPosition, 'id'>, id?: number) => {
        try {
            if (id) {
                const res: any = await put(`/academic-position/${id}`, data);
                setData((prev) => prev.map((p) => (p.id === id ? res : p)));
                setToast({ message: 'Academic Position updated successfully', type: 'success' });
            } else {
                const res: any = await post('/academic-position', data);
    ...
    const handleDelete = async (id: number) => {
        if (deleteId) {
            try {
                await del(`/academic-position/${id}`);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="m-6">
                ...
                <ConfirmDeleteDialog open={!!deleteId} onCancel={() => setDeleteId(null)} onConfirm={() => handleDelete(deleteId!)} />
                {toast && (
