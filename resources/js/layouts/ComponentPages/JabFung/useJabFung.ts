...
export const useJabatanFungsional = () => {
    ...
    const fetchData = async (currentPage = 1) => {
        try {
            setIsLoading(true);
            const res: any = await get(`/functional-position?page=${currentPage}&limit=10`);
    ...
    const handleSubmit = async (data: Omit<JabatanFungsional, 'id'>, id?: number, onSuccess?: () => void) => {
        try {
            setIsLoading(true);
            if (id) {
                const res: any = await put(`/functional-position/${id}`, data);
                setData((prev) => prev.map((p) => (p.id === id ? res.data : p)));
                await fetchData();
                onSuccess?.();
                setToast({ message: 'Jabatan Fungsional updated successfully', type: 'success' });
                return res;
            } else {
                const res: any = await post('/functional-position', data);
                setData((prev) => [...prev, res.data]);
                await fetchData();
                onSuccess?.();
                setToast({ message: 'Jabatan Fungsional created successfully', type: 'success' });
                return res;
            }
        } catch (error: any) {
            if (error?.response?.status === 500) {
                setToast({ message: 'Failed to submit Jabatan Fungsional', type: 'error' });
            }
    ...
    const handleDelete = async (id: number, onSuccess?: () => void) => {
        try {
            setIsLoading(true);
            await del(`/functional-position/${id}`);
