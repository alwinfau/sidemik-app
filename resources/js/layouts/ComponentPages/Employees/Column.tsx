...
export const columns = (
    onEdit: (row: EmployeesType) => void,   onDetail: (row: EmployeesType) => void, onDelete: (id: string) => void, selectedIds: number[], toggleSelect: (id: number) => void, toggleSelectAll: (checked: boolean) => void, allSelected: boolean): ColumnDef<EmployeesType>[] => [
    {
        id: 'select',
        header: ({ table }) => (
            <Checkbox
    ...
    // { accessorKey: 'back_title', header: 'Gelar Belakang' },
    {
        ...
        header: () => <div className="w-35">Nama Lengkap</div>,
        cell: ({ row }) => `${row.original.front_title ?? ''}  ${row.original.name} ${row.original.back_title}`,
    },
    {
        accessorKey: 'gender',
    ...
    // },
    {
        ...
        cell: ({ row }) => (
            <div className="flex gap-2">
                <Button className="bg-yellow-500 hover:bg-yellow-400" size="sm" onClick={() => onDetail(row.original)}>
                    <ListCollapse />
                </Button>
