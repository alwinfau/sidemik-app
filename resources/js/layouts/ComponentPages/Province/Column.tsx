import { Button } from "@/components/ui/button";
import { ColumnDef } from "@tanstack/react-table";
import { on } from "events";
import { Pencil, Trash2 } from "lucide-react";

export type ProvinceType = {
    id?: number;
    name: string;
    description: string;

};

export const columns = (onEdit: (row: ProvinceType) => void, onDelete: (id: string) => void): ColumnDef<ProvinceType>[] => [
    {
        id: "rowNumber",
        header: () => <div className="text-center">No</div>,
        cell: ({ row }) => <div className="text-center">{row.index + 1}</div>,
    },
    { accessorKey: "name", header: "Name" },
    { accessorKey: "dodescrtiption", header: "Document Description" },

    {
        id: "actions",
        header: "Actions",
        cell: ({ row }) => (
            <div className="flex gap-2">
                <Button className="bg-blue-700 hover:bg-blue-600" size="sm" onClick={() => onEdit(row.original)}>
                    <Pencil />
                </Button>
                <Button variant="destructive" size="sm" onClick={() => onDelete(row.original.id!.toString())}>
                    <Trash2 />
                </Button>
            </div>
        ),
    },
];

export default columns;